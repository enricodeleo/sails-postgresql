  ...
  var adapter = {
    ...
    registerDatastore: function registerDatastore(datastoreConfig, models, cb) {
      ...
      try {
        ...
        console.log('FOREIGN KEYS: Checking models for foreign keys');
        _.each(models, function(model, modelName) {
          console.log('FOREIGN KEYS: Checking model:', modelName);
          console.log('FOREIGN KEYS: Model definition keys:', Object.keys(model.definition || {}));
          
          _.each(model.definition, function(attribute, attrName) {
            console.log('FOREIGN KEYS: Checking attribute:', attrName, 'in model:', modelName);
            console.log('FOREIGN KEYS: Attribute type:', attribute.type, 'model:', attribute.model);
        ...
        if (hasForeignKeys) {
          ...
          // Debug model structure in detail
          Helpers.debugModels(models);
          
          // Make sure we're using the correct property for attributes
          _.each(models, function(model) {
            if (!model.schema && model.definition) {
              model.schema = model.definition;
            }
          });
